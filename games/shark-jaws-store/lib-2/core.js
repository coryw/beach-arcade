...
export function boot(opts = {}) {
  ...
  G.warm = (objects = []) => {
    const list = (Array.isArray(objects) ? objects : [objects]).map((o) => (o && !o.isObject3D && o.object?.isObject3D ? o.object : o)).filter((o) => o?.isObject3D);   // fx instances pass their .object
    const saved = [], added = [];
    for (const o of list) {
