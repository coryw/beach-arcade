...
export class Streaks {
  constructor(scene, { color, n = 80, box = [12, 8, -10, -120], length = 4, blending = "add" } = {}) {
    need("Streaks", "color", color); this.n = n; this.box = box; this.length = length;
    const k = blending === "normal" ? 4 : 3, pos = new Float32Array(n * 6), rgb = new Float32Array(n * 2 * k), c = col(color);   // normal: the tail fades by alpha, not to black
    for (let i = 0; i < n; i++) { rgb.set(k === 4 ? [c.r, c.g, c.b, 1, c.r, c.g, c.b, 0] : [c.r, c.g, c.b, 0, 0, 0], i * 2 * k); this._place(pos, i, box[3] + Math.random() * (box[2] - box[3])); }
    const g = new THREE.BufferGeometry(); g.setAttribute("position", (this.pa = dyn(pos, 3))); g.setAttribute("color", new THREE.BufferAttribute(rgb, k));
    this.material = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, blending: blendOf(blending), depthWrite: false });
